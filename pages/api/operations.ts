import { TransactionType, Csv, SingleMember, PurchaseOrSaleMember, DirectionType, CreditDebitType } from 'lib/Csv';
...
const makeRefinment = <T extends SingleMember > (transcationType : TransactionType): Refinement<SingleMember, T> =>
(x): x is T => x['Transaction Type'] === transcationType

const makeRefinmentDirection = <T extends SingleMember> (DirectionType : DirectionType): Refinement<SingleMember, T> =>
(x): x is T => x.Direction === DirectionType
...
export const getCollection = (description: string) => (collection: Array<Csv>) => 
...
export const getDiff = (debitSum: number, creditSum: number) => debitSum > creditSum? debitSum - creditSum : creditSum - debitSum;
...
export const convertBTCToCAD = (CADPrice : number) => (sum : number) => CADPrice * sum;
...
const getPriceByDate =  (date : string) : 
...
export const getCollectionDate = (description: string) => (collection: Array<Csv>) => 
    collection.map((rec: any) => {
        const  filtedRec = {
            Amount : rec[description],
            date : rec['Date']
